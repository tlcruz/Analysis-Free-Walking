...
% define spike beginning and ending by looking at zero-crossing of
% inflections in the speed signal
...
% extract spike properties in angular, forward and frequency
pV.peakVal = [];
pV.peakProm = [];
pV.vfstd = [];
pV.vfmeam = [];
...
for i = 1 : length(locs)
    if(locs(i)>spksIni(i) && locs(i)+spksEnd(i)<length(Vr))
        ...
        % if peaks don't cross the forward, angular and frequency criteria
        % they are discarded
